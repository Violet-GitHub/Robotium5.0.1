...
public final class ActivityThread {
	 ...
	 * 通过startActivity()或startActivityForResult()请求启动一个Activity时，
	 * 如果系统检测需要新建一个Activity对象时，就会回调handleLaunchActivity()方法，
	 * 该方法继而调用performLaunchActivity()方法，去创建一个Activity实例，
	 * 并且回调onCreate()，onStart()方法等，函数都位于 ActivityThread.java类 
     * @param r
     * @param customIntent
     */
	 ...
	 * 通过startService或者bindService时，如果系统检测到需要新创建一个Service实例，
	 * 就会回调handleCreateService()方法，完成相关数据操作。
	 * handleCreateService()函数位于 ActivityThread.java类
     * @param data
     */
	 ...
	 * 每个应用程序在第一次启动时，都会首先创建一个Application对象。
	 * 从startActivity流程可知，创建Application的实际在handleBindApplication()方法中
     */
