...
public class Instrumentation {
    ...
    /**
     * 控制instrumentation的运行。在应用程序代码加载之前，运行
    ...
    /**
     * 控制instrumentation的运行。在instrumentation运行时，创造或者开始一个新的thread
    ...
    /**
     * 控制instrumentation的运行。instrumentation开始执行方法
    ...
    /**
     * 控制instrumentation的运行。当系统捕捉到被应用抛出的未处理异常的时候调用
    ...
    /**
     * 同步方面的操作;在主线程中执行一个调用，主线程被阻滞直到调用结束 ；
     * 在主线程上执行一个任务，直到他完成。对于做事情不是线程安全的,比如查看或修改视图层次结构。
     ...
     * 在被测试app所在环境中，启动测试Activity，被测试的app包和测试Activity由intent传递
    ...
    /**
     * 有关特定的Intent的监视。一个ActivityMoniter类的实例通过函数addMonitor（Instrumentation.ActivityMonitor）
     * 添加到当前instrumentation中，一旦添加后，每当启动一个新的Activity，ActivityMoniter就会检测，如果匹配，
     * 其hit count计数更新等其他操作。 一个ActivityMonitor也可以用来寻找一个Activity，通过waitForActivity（）方法，
     * 这个函数将返直到匹配的活动被创建。
    ...
    /**
     * 添加ActivityMonitor，把一个Activitymonitor实例通过addMonitor方法添加到instrumentation中
     * 一旦添加后，每当启动一个新的Activity，ActivityMoniter就会检测，如果匹配，其hit count计数更新等其他操作。
